...
namespace Temp_Mail
{
    ...
    internal static class HTTPWrapper
    {
        ...
        {
            var xml = Get("http://api2.temp-mail.org/request/domains/format/xml/");
            return xml.Root.Elements().Select(x => x.Value).ToArray();
        }
        public static Letter[] GetLetters(string hash, string lastId = null)
        {
            var t = Get(string.Format("http://api2.temp-mail.org/request/mail/id/{0}/format/xml/", hash));
            if (t == null)
                return new Letter[0];
    ...
    /// <summary>
    /// API для работы с temp-mail.org
    /// </summary>
